...
use crate::{evaluation::piece_tables::material_score, search::SearchContext};

pub fn order_moves(ctx: &mut SearchContext, tt_move: Option<Move>, moves: &mut [Move]) {
    let last_moved_to = ctx.board.last_move().map(|m| m.to_square());
    ...
    moves.sort_by_cached_key(|m| {
        ...
        if Some(*m) == tt_move {
            // Highest priority (score here is not negated later)
            return i32::MIN;
        }
        ...
        if ctx
            .board
            .attacked_by_pawn(ctx.board.to_move(), m.to_square())
        {
